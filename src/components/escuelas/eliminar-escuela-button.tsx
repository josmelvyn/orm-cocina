'use client'
// src/components/escuelas/eliminar-escuela-button.tsx

import { useState, useTransition } from 'react'
import { useRouter } from 'next/navigation'
import { Trash2 } from 'lucide-react'
import { desactivarEscuelaAction } from '@/actions/escuelas.actions'
import { Button } from '@/components/ui/button'

export function EliminarEscuelaButton({ id, nombre }: { id: string; nombre: string }) {
  ...
  function handleEliminar() {
    ...
    startTransition(async () => {
      const resultado = await desactivarEscuelaAction(id)
